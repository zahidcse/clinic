...
@section('content')
    ...
    @include('appointments.consent.add_consent');
...
@push('script')
    ...
    <script>
        ...
        function updateuserConsentForm() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#deleteModal').modal('hide');
                    $('.selected-consent-list').html(data.content)
                },
                error: function(error) {
                    console.log(error);
                    $('#modal_confirm_delete').prop('disabled', false).text('Yes');
        ...
        $('.consent-all-list .consent-title').on('click', function(e) {

            let consent_id = $(this).data('consent_id');
            let consent_form_ids = $('#consent_form_ids').val().trim();

            consent_form_ids = consent_form_ids ? consent_form_ids.split(",") : [];

            if ($(this).hasClass("selected")) {
                $(this).removeClass("selected");
                consent_form_ids = consent_form_ids.filter(item => item !== String(consent_id));
            } else {
                $(this).addClass("selected");
                if (!consent_form_ids.includes(String(consent_id))) {
                    consent_form_ids.push(String(consent_id));
                }
            }
            $('#consent_form_ids').val(consent_form_ids.join(","));
        });
